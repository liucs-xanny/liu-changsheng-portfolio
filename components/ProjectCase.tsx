  ...
  return (
    <div className="space-y-5">
      <div className="relative aspect-[16/10] overflow-hidden rounded-[2rem] border border-paper/15">
        <Image src={layers[0]} alt="RGB原图" fill className="object-cover" />
        <div className="absolute inset-y-0 left-0 overflow-hidden" style={{ width: `${position}%` }}><div className="relative h-full" style={{ width: `${10000 / position}%` }}><Image src={layers[1]} alt="YOLO识别结果" fill className="object-cover" /></div></div>
      ...
      <div className="relative aspect-[16/9] overflow-hidden rounded-2xl"><Image src={layers[layer]} alt={["RGB图", "YOLO识别结果", "识别系统"][layer]} fill className="object-cover" /></div>
    </div>
  );
}
...
function LechangMap({ project }: { project: Project }) {
  ...
  return (
    <div className="grid gap-4 rounded-[2rem] bg-[#1d2f62] p-6 md:grid-cols-[1fr_.6fr]">
      <div className="relative aspect-square overflow-hidden rounded-2xl"><Image src={project.cover} alt="乐昌品牌地图" fill className="object-cover opacity-75" />{points.map((point, index) => <button key={point} onClick={() => setActive(index)} style={{ left: `${18 + index * 16}%`, top: `${25 + (index % 2) * 32}%` }} className={`absolute rounded-full border px-3 py-2 text-[10px] ${active === index ? "border-peach bg-peach text-ink" : "border-paper bg-ink/70"}`}>{index + 1}</button>)}</div>
  ...
  return (
    <main className="bg-ink text-paper">
      <section className="relative min-h-screen overflow-hidden">
        <Image src={project.cover} alt={project.title} fill priority className={`object-cover opacity-45 ${project.slug === "medical-robot" ? "object-top" : ""}`} />
